...
dataFile2 = open(sys.argv[3], 'r')
graphictype = int(sys.argv[4])
...
for line in dataFile2:
	...
	if(x != last_x):
		if(last_x != -1):
			graphic_fx2_med.append(fx_sum / i)
			graphic_x2_med.append(last_x)
			fx_sum = 0
			fx_fact_sum = 0
			i = 0
	last_x = x
...
if(graphictype == 0):
	...
	plt.plot(graphic_x, graphic_fx, 'bo', graphic_x_med, graphic_fx_med, 'b-', label="hoal")
	plt.plot(graphic_x2, graphic_fx2, 'go', graphic_x2_med, graphic_fx2_med, 'g-', label="hoal")
	plt.xlabel('x = cantidad de nodos en un grafo completo', fontsize=14)
	plt.ylabel('f(x) = diferencia entre w2 de la heuristica y la solucion exacta.\nVerde = golosa, Azul = busqueda local', fontsize=14)
	plt.title("(x, f(x))")
	plt.grid()
	plt.show()
