...
%En esta seccion se intentaron realizar los mismos dos tipos de analisis(evolucion absoluta y relativa de la funcion a minimizar) que en la busqueda %local, pero luego de realizar varias pruebas, con densidades bajas y altas de aristas en las instancias de test, vimos que no se obtienen mejoras con la busqueda local dada una solucion inicial golosa randomizada (aunque no sea lo mismo que greedy randomized, ver seccion de busqueda local con greedy como sol. inicial). Variamos tambien el parametro beta, pero no obtuvimos mejoras en iteraciones de GRASP.
