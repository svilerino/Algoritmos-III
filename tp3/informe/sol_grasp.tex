...
A continuaci\'on realizaremos el an\'alisis de complejidad te\'orica de una iteraci\'on de la heur\'istica GRASP. Dentro del ciclo principal, la primera instrucci\'on es generar una soluci\'on golosa aleatorizada inicial.
...
Acto seguido se valida si esta soluci\'on es factible, en tiempo constante, se declaran enteros y se procede a ejecutar la b\'usqueda local, cuya complejidad es de $O(k*n^w)$, siendo $k$ la cantidad de iteraciones hasta cumplida la condici\'on de terminaci\'on.
